...
class RDT  {
  ...
  throws IOException, InterruptedException{
    ...
    packetsRemainingInt = (int) Math.ceil( transmitMessage.length() / 80.0 ) +2 ; //+2 for the SYN and FIN
    ...
    while (packetsRemainingInt>1){
      ...
      if (transmitMessage.length()>95){
        newPacketData = transmitMessage.substring(0, 79); //first 94 bytes of message
        transmitMessage = transmitMessage.substring(79); //all after the first 95
        }else{
